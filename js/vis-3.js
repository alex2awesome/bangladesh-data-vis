var scrollVis = function () {
  ...
  var setUpMap = function (selection) {
    ...
    function makeMyMap(
      ...
      // Compute the bounds of a feature of interest, then derive scale & translate.
      var b = path_overview.bounds(country_mapping['Bangladesh']),
          s = .4 / Math.max(
            (b[1][0] - b[0][0]) / width, 
            (b[1][1] - b[0][1]) / height
          ),
          t = [
            -73 + (width  - s * (b[1][0] + b[0][0])) / 2, 
            223 + (height - s * (b[1][1] + b[0][1])) / 2
          ];
      ...
      label_bounds = path.bounds(country_mapping['Bangladesh']);
      console.log(label_bounds[0][0] + 800 )
      console.log(label_bounds[0][1] + 1000)
      svg.selectAll('.dhaka-metro-area-label')
        .data(['Dhaka','Metro','Area'])
        .enter().append('text')
        .attr('class', 'dhaka-metro-area-label')
        .attr('x', label_bounds[0][0] + 805)
        .attr('y', function(d, i) { return (label_bounds[0][1] + 1033) + (10 * i) })
      ...
      svg.append("rect")
        ...
        .attr("x", legend_bounds[0][0] - 3 )
        .attr("y", legend_bounds[0][1] - 16)
        .attr("width", (legend_bounds[1][0] - legend_bounds[0][0]) / .91 )
        .attr("height", (legend_bounds[1][1] - legend_bounds[0][1]) / .88 )
        .style("fill", "white")
      ...
      svg.append('text')
        ...
        .attr("x", legend_bounds[0][0] + 29 )
        .attr("y", legend_bounds[0][1] )
      ...
      svg.selectAll(".river-color-overview")
          .data(bangla_inland_rivers_poly.features)
        .enter().append("path")
          .attr("class", "river-color-overview")
          .attr("d", path_overview)
          .attr('opacity', .2)
          .style('fill', 'white')
      ;    
      ...
      svg.append("rect")
        .attr("class", "overview-locator")
        .attr("x", legend_bounds[0][0] + 44)
        .attr("y", legend_bounds[0][1] + 63)
        .attr("width", (legend_bounds[1][0] - legend_bounds[0][0]) / 5)
  ...
  var step_1 = function(){
      reShade({
        ...
        "overview-backdrop": .5,
        "overview-label": 1,
        "subunit-overview.BGD": 1,
        "river-color-overview": .2,
        "overview-locator": .7,
      }, 50)
  ...
  var step_2_activate = function(){
    ...
    // Elisha village
      reShade({
        ...
        "overview-backdrop": 0,
        "overview-label":0,
        "subunit-overview.BGD":0,
        "river-color-overview": 0,
        "overview-locator": 0
      }, 50)
  ...
  var step_2a_activate = function(){
    console.log('step 2a activating...')
  ...
  var step_3a_activate = function(){
    // Donia -> Elisha Ferry Terminal
    console.log('step 3a activating...')
      reShade({
  ...
  var setUpSections = function () {
    ...
    activateFunctions[2] = step_2a_activate
    activateFunctions[3] = step_3_activate
    activateFunctions[4] = step_3a_activate
    activateFunctions[5] = step_4_activate
    // activateFunctions[6] = step_4a_activate
    activateFunctions[6] = step_5_activate
    activateFunctions[7] = step_5a_activate
    activateFunctions[8] = step_6
    ...
    updateFunctions[2] = step_2_progress
    updateFunctions[4] = step_3_progress
    updateFunctions[5] = step_4_progress
    updateFunctions[7] = step_5_progress
  };
